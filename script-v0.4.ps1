        ...
        foreach($file in $baselineFiles){
            if(Test-Path -Path $file.path){
                ...
                if($currenthash.Hash -eq $file.hash){
                    ...
                    if($emailTo){
                        Send-MailKitMessage -To $emailTo -From $emailCred.username -Subject "File Monitoring system, file has changed !!" -Body "$($file.path) Some thing is changed!!" -SMTPServer $emailServer -Port $emailPort -Credential $emailCred 
                    }
                }
            }
            else{
...
do{
    Write-Host "Select any of the following options to start or type q to quit" -ForegroundColor green
    ...
    switch ($entry) {
        "1"{
            $baselineFilePath = Read-Host -Prompt "Enter Baseline File path:"
            if(Test-Path -Path $baselineFilePath){
        ...
        "2"{
            $targetFilePath = Read-Host -Prompt "Enter The Path of the file you want to moniter"
        ...
        "5"{
            $newBaselineFilePath = Read-Host -Prompt "Enter file for the new baseline file"
